...
// const scenes = @import("scene_manager.zig");
...
pub fn main() anyerror!void {
    ...
    while (!raylib.windowShouldClose()) {
        ...
        //----------------------------------------------------------------------------------
        {
            ...
            raylib.clearBackground(.light_gray);
            ...
            raylib.drawText("- Right/Left to move", 40, 40, 10, .dark_gray);
            // todo: controls text
            raylib.drawText("- Current camera mode:", 20, 120, 10, .black);
            raylib.drawText(camera_descriptions[camera_option], 40, 140, 10, .dark_gray);
        }
...
fn updatePlayer(player: *Player, env_items: []EnvItem, delta: f32) void {
    if (raylib.isKeyDown(.left)) player.position.x -= PLAYER_HOR_SPD * delta;
    if (raylib.isKeyDown(.right)) player.position.x += PLAYER_HOR_SPD * delta;
    if (raylib.isKeyDown(.space) and player.can_jump) {
...
fn updatecameraCenterInsideMap(
    ...
    const max = raylib.getWorldToScreen2D(Vec2.init(max_x, max_y), camera.*);
    const min = raylib.getWorldToScreen2D(Vec2.init(min_x, min_y), camera.*);
...
fn updateCameraPlayerBoundsPush(
    ...
    const bbox_world_min = raylib.getScreenToWorld2D(Vec2.init((1 - bbox.x) * 0.5 * widthf, (1 - bbox.y) * 0.5 * heightf), camera.*);
    const bbox_world_max = raylib.getScreenToWorld2D(Vec2.init((1 + bbox.x) * 0.5 * widthf, (1 + bbox.y) * 0.5 * heightf), camera.*);
